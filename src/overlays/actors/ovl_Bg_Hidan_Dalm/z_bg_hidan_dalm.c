...
void BgHidanDalm_Wait(BgHidanDalm* this, PlayState* play) {
    ...
    if ((this->collider.base.acFlags & AC_HIT) && !Player_InCsMode(play) &&
        (player->meleeAttackType == PLAYER_MELEEATKTYPE_HAMMER_FORWARD ||
         player->meleeAttackType == PLAYER_MELEEATKTYPE_HAMMER_SIDE)) {
        ...
        Actor_SetPlayerCutscene(play, &this->dyna.actor, PLAYER_CSMODE_WAIT);
...
void BgHidanDalm_Shrink(BgHidanDalm* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->dyna.actor.scale.x, 0.0f, 0.004f)) {
        Actor_SetPlayerCutscene(play, &this->dyna.actor, PLAYER_CSMODE_END);
