    ...
    if (this->introState != BFD_CS_NONE) {
        ...
        switch (this->introState) {
            ...
            case BFD_CS_LOOK_LINK:
                ...
                if (this->timers[0] == 40) {
                    Actor_SetPlayerCutscene(play, &this->actor, PLAYER_CSMODE_REACT_TO_QUAKE);
                }
                if (this->timers[0] == 0) {
            ...
            case BFD_CS_LOOK_GROUND:
                ...
                if (this->timers[0] == 0) {
                    ...
                    this->subCamVelFactor = 0.0f;
                    this->subCamAccel = 0.0f;
                    Actor_SetPlayerCutscene(play, &this->actor, PLAYER_CSMODE_END_REACT_TO_QUAKE);
                }
                break;
        ...
        if (effect->type != BFD_FX_NONE) {
            ...
            if (effect->type == BFD_FX_EMBER) {
                ...
                if ((this->timers[3] == 0) && (sqrtf(SQ(diff.x) + SQ(diff.y) + SQ(diff.z)) < 20.0f)) {
                    this->timers[3] = 50;
                    Actor_SetPlayerKnockbackDamage(play, NULL, 5.0f, effect->kbAngle, 0.0f, 0x30);
