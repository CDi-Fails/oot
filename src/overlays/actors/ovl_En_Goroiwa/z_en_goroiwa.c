...
void EnGoroiwa_Roll(EnGoroiwa* this, PlayState* play) {
    ...
    if (this->collider.base.atFlags & AT_HIT) {
        ...
        Actor_SetPlayerKnockbackDamage(play, &this->actor, 2.0f, this->actor.yawTowardsPlayer, 0.0f, 0);
...
void EnGoroiwa_MoveUp(EnGoroiwa* this, PlayState* play) {
    if (this->collider.base.atFlags & AT_HIT) {
        this->collider.base.atFlags &= ~AT_HIT;
        Actor_SetPlayerKnockbackDamage(play, &this->actor, 2.0f, this->actor.yawTowardsPlayer, 0.0f, 4);
        func_8002F7DC(&GET_PLAYER(play)->actor, NA_SE_PL_BODY_HIT);
        if ((this->actor.home.rot.z & 1) == 1) {
            this->collisionDisabledTimer = 50;
        }
...
void EnGoroiwa_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(player->stateFlags1 & (PLAYER_STATE1_TALKING | PLAYER_STATE1_IN_DEATH_CUTSCENE | PLAYER_STATE1_SKIP_OTHER_ACTORS_UPDATE | PLAYER_STATE1_IN_CUTSCENE))) {
