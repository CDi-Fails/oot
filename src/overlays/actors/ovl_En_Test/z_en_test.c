    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (this->actor.xzDistToPlayer < 100.0f) {
            if ((player->isMeleeWeaponAttacking != 0) && (ABS(yawDiff) >= 0x1F40)) {
                ...
                if (Rand_ZeroOne() > 0.7f && player->meleeAttackType != PLAYER_MELEEATKTYPE_JUMPSLASH_START) {
                    EnTest_SetupJumpBack(this);
                } else {
                    func_808627C4(this, play);
                }
        ...
        if (this->actor.colChkInfo.health == 0) {
            ...
            if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) &&
                ((ABS((s16)(this->actor.wallYaw - this->actor.shape.rot.y)) < 0x38A4) &&
                 (this->actor.xzDistToPlayer < 80.0f))) {
                EnTest_SetupJumpUp(this);
            } else if ((Rand_ZeroOne() > 0.7f) && (player->meleeAttackType != PLAYER_MELEEATKTYPE_JUMPSLASH_START)) {
                EnTest_SetupJumpBack(this);
            } else {
                EnTest_SetupStopAndBlock(this);
            }
...
void EnTest_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_FIREMAGIC) {
        ...
        if (this->actor.params == STALFOS_TYPE_1) {
            ...
            if ((floorProperty == FLOOR_PROPERTY_VOID_PIT) || (floorProperty == FLOOR_PROPERTY_VOID_PIT_LARGE) ||
                SurfaceType_GetFloorType(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId) == FLOOR_TYPE_VOID_ON_TOUCH) {
                Actor_Kill(&this->actor);
                return;
            }
        }
